...
extensions_v1 = client.ExtensionsV1beta1Api()
...
def find_unused_pvs():
    ...
    for pv in v1.list_persistent_volume().items:
        if pv.status.phase == "Available" and not pv.spec.claim_ref:
...
# Function to find unused HorizontalPodAutoscalers
def find_unused_hpas():
    unused_hpas = []
    for ns in NAMESPACES or [ns.metadata.name for ns in v1.list_namespace().items]:
        unused_hpas.extend([hpa.metadata.name for hpa in autoscaling_v2.list_namespaced_horizontal_pod_autoscaler(ns).items])
    return unused_hpas
